...
@testset "Constructors" begin
    ...
    @test @inferred(Column(a,v1)) == Column{a,Int64,Vector{Int64}}(v1)
    @test @inferred(Column(a)) == Column{a,Int64,Vector{Int64}}(Vector{Int64}())
    @test @inferred(Column(c1,c2,c3)) == Column{a,Int64,Vector{Int64}}(v1)
    @test @inferred(Column(a,1,2,3)) == Column{a,Int64,Vector{Int64}}(v1)
    @test @inferred(a([1,2,3])) == Column{a,Int64,Vector{Int64}}(v1)
...
@testset "Introspection" begin
    ...
    @test @inferred(rename(col1,a_new)) == @column a_new = v1
...
@testset "Composing and concatenating" begin
    ...
    @test @inferred(vcat(c1)) == Column(c1)
    @test @inferred(vcat(c1,c2,c3)) == col1
    @test @inferred(vcat(col1,c1)) == @column a = [1,2,3,1]
    @test @inferred(vcat(c1,col1)) == @column a = [1,1,2,3]
